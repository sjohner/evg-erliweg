...
- Q: How should quarterly data be updated securely and simply? -> A: Maintainers update the repository data file and push the change directly to `main`; the GitHub Pages workflow then publishes the pushed state.
...
- Q: Should energy data updates have an automated data integrity release gate? -> A: No; the single-maintainer workflow uses direct pushes to `main`, with corrections made through a follow-up fix-forward push.
...
5. **Given** a visitor wants to inspect the project source, **When** they activate the GitHub icon in the header, **Then** they are directed to the public repository at https://github.com/sjohner/evg-erliweg.
6. **Given** a visitor views the header, **When** the repository link and theme switcher are displayed, **Then** the website title remains left-aligned while the repository link and dark-mode toggle remain right-aligned together on the same horizontal level, with the repository link immediately to the left of the toggle and both controls keyboard accessible.
...
As a maintainer, I want to update produced and consumed energy values in a structured data file in the public repository so published figures stay current without a separate admin area.
...
**Independent Test**: Can be fully tested by editing one quarterly record, pushing the change directly to `main`, and verifying the GitHub Pages workflow publishes the new values.

**Acceptance Scenarios**:

1. **Given** a maintainer has repository write access, **When** they update quarterly production and consumption values and push the change directly to `main`, **Then** those values become the official record for that quarter.
2. **Given** quarterly values are pushed to `main`, **When** the GitHub Pages workflow completes, **Then** the current and historical summaries reflect the latest pushed values.
...
- How does the site behave when GitHub or the public repository is temporarily unavailable?
...
- **FR-012**: System MUST store quarterly production and consumption values in a
	structured JSON or YAML data file within the public repository.
...
- **FR-014**: System MUST clearly show the reporting period associated with each
	displayed metric.
- **FR-015**: System MUST support publication by pushing data-file changes directly
	to `main`, after which the GitHub Pages workflow publishes the pushed state.
...
- **SC-010**: At 390px, 430px, 768px, and 1280px viewport widths, the website title remains left-aligned and vertically aligned with the right-aligned GitHub repository link and dark-mode toggle; the two 44 by 44 CSS pixel actions remain in order without overlap or horizontal overflow and can be identified and activated using pointer and keyboard input.
...
- Maintainers with write access to the public repository are the authorized updaters of quarterly data.
- The single-maintainer quarterly update flow uses direct pushes to `main`; number corrections use a follow-up fix-forward push.
- Website publication runs through the GitHub Pages workflow after data-file changes are pushed to `main`.
...
- The project's canonical public repository URL is https://github.com/sjohner/evg-erliweg; GitHub availability remains outside this website feature's scope.
